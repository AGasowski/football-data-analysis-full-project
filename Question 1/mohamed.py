...
buts_encaissés_domicile =
df.groupby("home_team_api_id")["away_team_goal"].sum() buts_encaissés_extérieur
= df.groupby("away_team_api_id")["home_team_goal"].sum()

buts_totaux_encaissés = buts_encaissés_domicile.add(buts_encaissés_extérieur,
fill_value=0)

equipe_moins_buts = buts_totaux_encaissés.idxmin()

nom_equipe = teams.loc[teams["team_api_id"] == equipe_moins_buts,
"team_long_name"].values[0]]
