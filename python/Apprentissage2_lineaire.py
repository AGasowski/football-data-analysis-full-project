...
def predire_classement_avec_confiance(saison, league_id_cible, team_id_cible):
    ...
    match = match[match["league_id"] == league_id_cible]  # Filtrer la ligue souhaitée
    ...
    def extraire_caracteristiques_mi_saison(df_season):
        ...
        equipes = pd.unique(df_season[["home_team_api_id", "away_team_api_id"]].values.ravel())
    ...
    def calculer_rank_final(df_season, equipes):
        ...
        df_final = pd.DataFrame(list(total_points.items()), columns=["equipe", "points_fin_saison"])
        df_final = df_final.sort_values(by="points_fin_saison", ascending=False).reset_index(drop=True)
    ...
    def ajouter_moyenne_rang_adversaires_restants(df_stats, df_season):
        ...
        for team_id in df_stats["equipe"]:
            ...
            opponents = [
                row["away_team_api_id"] if row["home_team_api_id"] == team_id else row["home_team_api_id"]
                for _, row in remaining.iterrows()
            ]
            ranks = [rank_dict.get(oppo) for oppo in opponents if oppo in rank_dict]
    ...
    def preparer_donnees_saison(season):
        ...
        df_stats = ajouter_moyenne_rang_adversaires_restants(df_stats, df_season)
    ...
    saisons = ["2008/2009", "2009/2010", "2010/2011", "2011/2012", "2012/2013", "2013/2014"]
    df_all = pd.concat([preparer_donnees_saison(s) for s in saisons], ignore_index=True)

    # Étape 4 : Préparer les données pour l'apprentissage
    features = ["points", "goal_diff", "buts_marques", "buts_encaisses", "victoires",
