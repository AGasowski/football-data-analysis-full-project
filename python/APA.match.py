...
def match_is_valid(row):
    return (
        (row['saison'], row['home_team_api_id']) in valid_teams and
        (row['saison'], row['away_team_api_id']) in valid_teams
    )

# On applique ce filtre
matches_filtered = matches[matches.apply(match_is_valid, axis=1)]
match = matches_filtered[['league_id','saison','home_team_api_id','away_team_api_id','home_team_goal','away_team_goal']]
match.to_pickle('match.pkl')
