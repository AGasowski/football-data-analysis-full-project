...
match= select_colonnes(match,["home_team_api_id","home_team_goal","away_team_goal","shoton"])
...
match["tir_cadre"]=match['shoton'].map(lambda x: len(x) if x not in [''] and len(x)>0 else 1) 
