...
def carton_rouge(season):
    ...
    results = []
    for team_id, (nb_red_cards, nb_matches) in team_stats.items():
        ratio = round(nb_red_cards / nb_matches, 2) if nb_matches > 0 else 0
        results.append(
            {
                "team_api_id": int(team_id),
                "season": season,
                "red_cards_per_match": ratio,
...
def carton_rouge_toutes_saisons():
    # Charger les données
    match = lire_csv("data/Match.csv")
...
print(carton_rouge_toutes_saisons())
