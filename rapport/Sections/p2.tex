...
semble être une statistique inutile à première vue, mais nous souhaitions
observer si cette taille était élevée, ou bien similaire à la taille moyenne
d'une population de joueurs de foot. Comme pour les questions précédentes, nous
avons permis à l'utilisateur de choisir une saison en particulier ou non. De
plus, nous l'avons traitée avec pandas.
...
accessibles sur les sites spécialisés puisqu'elles sont au coeur des débats
...
\textbf{Question 5 : Combien de fois ont été utilisé chaque dispositif ?}

\vskip 10pt

Au départ, nous voulions connaître le dispositif le plus utilisé, et qui
semblerait être le préféré des entraîneur. La réponse renvoyée était la même
pour chaque saison: 4 2 3 1. Nous avons alors eu envie de connaître les autres
dispositifs les plus utilisés, d'où la création de ce classement. Il est
important de rappeler que la base de donnée ne prend pas en compte les
changements possibles de dispositif au cours d'un match. La réalisation de
cette question a été faite en utilisant pandas.
...
avons utilisé le paquet pandas. L'idée était de trouver ce jour un peu
exceptionnel, par curiosité. Il s'agit d'un samedi , ce qui semble logique
puisque plus de matchs y sont joués. Le paquet pandas nous a de nouveau servi
ici.
...
\begin{itemize}
    \item \textbf{Choix des critères les plus importants pour jouer à un
...
L'utilisateur a ici le choix de choisir la saison pour laquelle il souhaite
obtenir l'équipe type. Pour une visualisation plus agréable, nous avons
modélisé un beau terrain de foot afin de placer les joueurs au bon endroit.
Toute cette question a été réalisée à l'aide du paquet pandas. 
...
l'équipe qui joue à domicile. Cependant, certaines équipe ont de meilleurs
...
L'idée était de connaître l'équipe avec le moins de résussite, par rapport à
ses frappes cadrées. En d'autres termes, quelle équipe a du faire face à des
gardiens très efficaces ? L'utilisateur a de nouveau la possibilité de choisir
la saison pour laquelle il souhaite voir cette statique. Cette question a été
réalisée en python pur.