...
confondues, ou bien pour une saison bien particulière. De même, il peut choisir
de réaliser la recherche pour un championnat en particulier ou pour tous les
championnats confondus.
...
choisir de trouver ce jour pour une saison en particulier. Le paquet pandas
nous a de nouveau servi ici.
...
\begin{itemize}
    ...
    présent dans notre équipe type. Soit un poste $P$ (par exemple défenseur
    central), et un ensemble de critères $C_P = \{c_1, c_2, \dots, c_n\}$
    considérés comme les plus pertinents pour ce poste.

    Chaque joueur $j$ possède une note $s_j(c_k)$ pour chaque critère $c_k \in
    C_P$.

    Le \textbf{score total} $S_j$ du joueur $j$ pour le poste $P$ est défini
    comme la somme des notes sur les critères sélectionnés :

    \[
    S_j = \sum_{k=1}^{n} s_j(c_k)
    \]

    où :
    \begin{itemize}
        \item $S_j$ : score total du joueur $j$ pour le poste $P$,
        \item $s_j(c_k)$ : note du joueur $j$ pour le critère $c_k$,
        \item $n$ : nombre de critères considérés pour le poste.
    \end{itemize}

    Le joueur sélectionné pour le poste $P$ est celui qui maximise $S_j$, tout
    en respectant la contrainte qu’il ne soit pas déjà présent dans l’équipe
    type :

    \[
    j^* = \underset{j \notin \text{équipe}}{\arg\max} \; S_j
    \]

    Pour les postes où le pied fort est pertinent (latéral, ailier, défenseur
    central), une contrainte supplémentaire peut être ajoutée pour ne retenir
    que les joueurs dont le pied fort correspond au côté du terrain :

    \[
    j^* = \underset{\substack{j \notin \text{équipe} \\ \text{pied}_j = 
    \text{côté}_P}}{\arg\max} \; S_j
    \]
...
a été réalisée à l'aide du paquet pandas. 
...
L'analyse a été réalisée en Python pur.
