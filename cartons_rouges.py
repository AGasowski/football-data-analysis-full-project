...
carton_rouges = {name: g.get(id) for name, id in d3.items() if g.get(id) is not None}
meilleur_carton_rouges = sorted(carton_rouges.items(), key=lambda x: x[1], reverse=True)[:30]
Classement_meilleurs_rouges = pd.DataFrame(meilleur_carton_rouges, columns=["player_name", "nb_carton jaune"])
